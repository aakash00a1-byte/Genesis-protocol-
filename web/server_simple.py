...
def api_chat():
    ...
    # Direct Groq API call (standalone mode)
    ...
    if api_key:
        try:
            ...
            if resp.status_code == 200:
                ...
                model_used = result.get('model', 'llama-3.1-8b-instant')
                provider = "groq"
                quality = 0.85
                mode = "fast"
            else:
                response_text = f"Groq API error: {resp.status_code}"
        except Exception as e:
            logger.warning(f"Groq API error: {e}")
            response_text = f"Error: {str(e)}"
    else:
        response_text = "AI not configured. Please set GROQ_API_KEY environment variable."
